...
import {Row, Col, Input, Switch, Slider, Radio, DatePicker, TimePicker, Drawer, Button, notification} from "antd"
...
const EditEvent = ({event, editEvent, setEditEvent, journeyStart, journeyDur, journeyType}) => {
			...
			title={
				<Row>
					<Col span={12} className="flex items-center space-x-4">
						<h1 className="text-[20px] font-semibold leading-[28px] text-[#262626]">Touchpoint</h1>
						<Button
							size="small"
							className="bg-[#FF4D4F] text-white hover:border-none hover:bg-[#FF4D4F] hover:text-white"
							onClick={onDelete}
						>
							Delete
						</Button>
					...
					<Col span={12} className="flex items-center justify-end">
						<ActionButtons onCancel={() => setEditEvent(false)} onSubmit={updateEvent} />
					</Col>
				</Row>
			}
			placement={"bottom"}
