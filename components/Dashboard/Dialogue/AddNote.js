...
import {Row, Col, Button, Input, Form, Drawer} from "antd"
...
const AddNote = ({visible, setVisible, onSubmit, height = 378}) => {
	const ref = useRef()
		...
		<Drawer
			...
			title={
				<div className="flex justify-between gap-4">
					<Input value={item.name} placeholder="John Doe" onChange={handleName} className="max-w-sm" />
					<ActionButtons className="justify-end" onCancel={onClose} onSubmit={handleFinish} />
				</div>
			}
		>
			<Row gutter={[20, 20]}>
				<Col span={16}>
					...
					<Form ref={ref} onFinish={handleFinish}>
						...
						<Button className="border-[#4A801D] text-[#4A801D]" onClick={onAddNote}>
							Add More
						</Button>
					</Form>
				</Col>
				<Col offset={3} span={4}>
					...
					<Input className="text-right text-lg" value={item.region} placeholder="Africa" onChange={handleRegion} />
