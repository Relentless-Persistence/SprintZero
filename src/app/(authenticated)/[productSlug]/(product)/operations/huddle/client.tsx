								...
								<div>
									...
									<div className="mt-1 flex flex-col gap-2">
										...
										{huddleUser.id === user.id &&
											storyMapItems &&
											(() => {
												const blockerStoryOptions = getStories(storyMapItems).filter(
													({id}) => !huddle?.data().blockerStoryIds.includes(id),
												)
								...
								<div>
									...
									<div className="mt-1 flex flex-col gap-2">
										...
										{huddleUser.id === user.id &&
											storyMapItems &&
											(() => {
												const todayStoryOptions = getStories(storyMapItems).filter(
													({id}) => !huddle?.data().todayStoryIds.includes(id),
												)
								...
								<div>
									...
									<div className="mt-1 flex flex-col gap-2">
										...
										{huddleUser.id === user.id &&
											storyMapItems &&
											(() => {
												const yesterdayStoryOptions = getStories(storyMapItems).filter(
													({id}) => !huddle?.data().yesterdayStoryIds.includes(id),
												)
