...
const Epic: FC<EpicProps> = ({ storyMapItems, epicId }) => {
  ...
  const x = useMotionValue(epic.roadmap?.x)
  const y = useMotionValue(epic.roadmap?.y)

  useEffect(() => {
    x.set(epics.find(({ id }) => id === epic.id)!.roadmap?.x)
    y.set(epics.find(({ id }) => id === epic.id)!.roadmap?.y)
  }, [epic.id, epics, x, y])
